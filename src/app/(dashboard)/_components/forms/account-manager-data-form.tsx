...
const invoicesCsvRequiredHeaders = [
  ...
  "Amount invoiced",
  "Month",
  "Account manager is the deal owner",
] as const;
...
export default function AccountManagerDataForm() {
  ...
  const addMonthlyData = api.accountManager.addMonthlyData.useMutation({
    onSuccess: (res) => {
  ...
  const deleteDataUptoLockedMonth =
    api.accountManager.deleteDataUptoLockedMonth.useMutation({
      onSuccess: (res) => {
        toast.success(res.message);
      },
      onError: (err) => {
    ...
    <div className="max-w-2xl">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Import invoices and collections for a given month. Accepted format:
            CSV.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                Required columns:
                <br />
                Deal ID, Deal link, Deal name, Account Manager Name, Account
                Manager Email, Amount invoiced, Month, Account manager is the
                deal owner
              </p>
              <p className="text-xs text-muted-foreground">
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                Required columns:
                <br />
                Deal ID, Account Manager Name, Account Manager Email, Amount
                paid, Month
              </p>
              <p className="text-xs text-muted-foreground">
                Example CSV filename:
